# go-ruby-kafka documentation

**Pure-Go (CGO=0), MRI-faithful reimplementation of the ruby-kafka gem's client surface — producer, consumer and topic-admin over Apache Kafka.**

`go-ruby-kafka/kafka` is a faithful, pure-Go (zero cgo) reimplementation of Ruby's `kafka`,
matching reference Ruby (MRI) behaviour. The module path is
`github.com/go-ruby-kafka/kafka`.
...
!!! success "Status: pure-Go, CGO=0, differential-tested"
    A faithful pure-Go port of Ruby's `kafka`, validated against reference Ruby, at 100%
...
```sh
go get github.com/go-ruby-kafka/kafka
```
...
| Repo | What it is |
| --- | --- |
| [`kafka`](https://github.com/go-ruby-kafka/kafka) | the library — Ruby's `kafka` in pure Go |
| [`docs`](https://github.com/go-ruby-kafka/docs) | this documentation site (MkDocs Material, versioned with mike) |
| [`go-ruby-kafka.github.io`](https://github.com/go-ruby-kafka/go-ruby-kafka.github.io) | the organization landing page (Hugo) |
| [`brand`](https://github.com/go-ruby-kafka/brand) | logo and brand assets |
...
Source lives at [github.com/go-ruby-kafka/kafka](https://github.com/go-ruby-kafka/kafka).
