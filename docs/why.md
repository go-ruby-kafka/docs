...
`go-ruby-kafka/kafka` reimplements Ruby's `kafka` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-kafka/kafka` directly, with no Ruby runtime;
